...
public class ValidateUploadFilter implements Filter {
	// 对HttpServletRequest进行读取时，只能读取一次。
	// 如果Filter调用getInputStream()读取了一次数据，后续Servlet处理时，再次读取，将无法读到任何数据。怎么办？
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (digest == null || digest.length() == 0 || signature == null || signature.length()==0) {
